...
To complete Part One, I first gathered the annual SEC financial values for 3M, P&G, and Walmart for fiscal years 2016 through 2021 and entered those figures into the Excel workbook on the `Data` tab. That tab contains the base inputs used for the analysis: accounts receivable, inventory, accounts payable, annual sales, and annual cost of goods sold for each company and year. I then used the workbook to organize the data in a calculation-friendly format and verify the working capital metrics. The formulas applied were standard spreadsheet formulas: days of accounts receivable equals `(Accounts Receivable / Annual Sales) * 365`, days of inventory equals `(Inventory / Annual COGS) * 365`, days of accounts payable equals `(Accounts Payable / Annual COGS) * 365`, and cash conversion cycle equals `DAR + DI - DAP`.

To make the workbook easier to audit, I also used a separate `Calculations` tab that references the `Data` tab and recomputes each metric by company and year. This creates a clean trail from the raw financial inputs to the final working capital measures shown in the assignment. For readability, the financial statement values shown in the completed `Data` tab are presented in USD millions, while the supporting calculation tab converts those same values into full USD amounts and uses Excel formulas to calculate DAR, DI, DAP, and CCC directly. This way, the spreadsheet clearly shows both the source values and the formulas used to produce the final results.
